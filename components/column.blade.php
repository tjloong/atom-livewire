...
<th {{ $attrs }}>
    ...
    class="py-1.5 px-3 inline-flex items-center gap-2">
        ...
        @if ($sort)
            <atom:icon down x-show="sorted && (sort.direction === 'asc' || !sort.direction)" class="shrink-0 text-zinc-400"/>
            <atom:icon up x-show="sorted && sort.direction === 'desc'" class="shrink-0 text-zinc-400"/>
            <atom:icon dropdown x-show="!sorted" class="shrink-0 text-zinc-400"/>
        @endif
    </div>
</th>
