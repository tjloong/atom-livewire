<x-form modal icon="paper-plane" uid="email-modal" header="Send Email">
    @if ($inputs)
        ...
        <x-form.group>
            <x-form.email wire:model="inputs.to" :options="data_get($settings, 'options')" multiple/>
            <x-form.email wire:model="inputs.cc" :options="data_get($settings, 'options')" multiple/>
